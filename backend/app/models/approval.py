...
class Approval(Base):
    ...
    requester_org_id: Mapped[Optional[str]] = mapped_column(
    ...
    target_org_id: Mapped[Optional[str]] = mapped_column(
        ForeignKey("organizations.id"), nullable=True
    )
    ...
    def requester_name(self) -> str:
        return (self.requester.display_name or self.requester.username) if self.requester else "Unknown"
